...
|:--|:--|
| グループ共通部品（名称） | パッシブブザー |
| 共通部品が担う役割 | ゲーム中の音を鳴らす |
| 接続するArduinoピン（予定） | D8（仮） |
| 個人の設計と共通部品が協調する箇所 | ジョイスティック操作で音程・長さを変え、正解時にブザーを鳴らす |
...
補足：ジョイスティックの上下で音程、左右で長さを調整。正解時にLED点灯。
...
|:--|:--|:--|
| 待機中 | スタート待ち | LED消灯 |
| 演奏中 | ランダム音再生・操作受付 | ブザー音、LED点滅 |
| 結果表示 | 判定・結果表示 | 正解時LED点灯、不正解時消灯 |
...
| lastMillis | millis()用タイマー | unsigned long | 4B | 0 | |
...
|:--|:--|:--|:--|:--|
| パッシブブザー | 標準 | D8 | 出力 | |
| ジョイスティック | 標準 | A0/A1 | 入力 | X: A0, Y: A1 |
| 白色LED | 5mm | D9 | 出力 | 220Ω抵抗直列 |
