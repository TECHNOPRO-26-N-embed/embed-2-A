...
|debounceDelay  |デバウンス判定の待ち時間  |const unsigned long  | 4B | 50 |50ms程度  |
> [!CAUTION]
...
| F02	| 集中モード終了	| endFocusMode()	| 集中モードを終了状態に切り替える	| なし	| なし	| loop()内|
> [!CAUTION]
...
|センサー読み取り  |100ms  | millis |連続して値を確認したい  |
...
|LED  | RGB_LED |D9  |出力  |330Ω抵抗と直列  |
|ブザー  | Passive Buzzer |D10  |出力  | 様々な状態に応じて鳴動 |
|  |  |  |  |  |
...
|  配線不良|部品が反応しない  |値が常に0/想定外  | 初期状態へ戻し、再接続確認 |
|  |  |  |  |
